...
from iced_x86 import Code, Instruction, InstructionInfoFactory, MemoryOperand, Register, BlockEncoder, Decoder
...
def detect_version(page_data: bytearray) -> tuple[str, str, str]:
    ...
    if not result:
        raise DataNotFound("Failed to detect Under a Killing Moon or The Pandora Directive! Please create an issue on https://github.com/moralrecordings/tex3-mouselook")
    game, version = result[0].group(1).decode('ascii'), result[0].group(2).decode('ascii')
    ...
    if game not in ("Under a Killing Moon", "The Pandora Directive"):
        raise DataNotFound(f"Unknown game {game}, must be one of \"Under a Killing Moon\" or \"The Pandora Directive\"")

    print(f"Found {game} v{version}, {language.title()} language")
    return game, version, language


def find_offset(page_data: bytearray, pattern: str, offset: int, description: str) -> int:
    if not pattern:
    ...
    if len(matches) > 1:
        raise DataNotFound(f"Multiple offset matches found for {description} ({matches}), aborting")
...
def find_variable(page_data: bytearray, pattern: str, description: str) -> int:
    ...
    if len(matches) > 1:
        raise DataNotFound(f"Multiple variable matches found for {description} ({matches}), aborting")
...
def patch(input: pathlib.Path, output: pathlib.Path, fix_speed: bool, mouselook: bool) -> None:
...
"""
        fix_speed_offset = find_offset(page_data, "\\xf7\\xd8\\x83\\xc0\\x64\\x75\\x05\\xb8\\x04\\x00\\x00\\x00", 5, "speed bug code")
    ...
    if mouselook:
        ...
        var_movement_rot_angle = find_variable(page_data, "\\xa3(.{4})\\xc1\\xf8\\x10\\xe8.{4}\\xa1.{4}", "head rotation angle")
        var_movement_tilt_angle = find_variable(page_data, "\\xc7\\x05(.{4})\\x2c\\x01\\x00\\x00", "head tilt angle")
        var_movement_tilt_angle_last = find_variable(page_data, "\\xa3(.{4})\\xa1.{4}\\x0b\\xc0\\x74\\x2c", "last head tilt angle")
        var_movement_tilt_angle_bottom = find_variable(page_data, "\\xa1(.{4})\\xa3.{4}\\xa3.{4}\\x0f\\xb6\\x1d.{4}", "min head tilt angle")
        var_movement_tilt_angle_top = find_variable(page_data, "\\xa1(.{4})\\xa3.{4}\\xa3.{4}\\xa1.{4}\\x0b\\xc0", "max head tilt angle")
        var_movement_strafe = find_variable(page_data, "\\x83\\x25(.{4})\\xfc\\x66\\x0f.{4}", "strafe flag")
        var_keyboard_state = find_variable(page_data, "\\xb9\\x2c\\x00\\x00\\x00\\xbf(.{4})", "keyboard state array")
        var_movement_fwd_veloc_world = find_variable(page_data, "\\xf7\\x2d.{4}\\x0f\\xac\\xd0\\x10\\xa3(.{4})\\x8b\\xc1", "forward velocity")
        var_movement_strafe_veloc_world = find_variable(page_data, "\\x0b\\xed\\x79\\x02\\xf7\\xd8\\xa3(.{4})\\xc3", "strafe velocity")
        var_movement_eye_level_incr = find_variable(page_data, "\\x80\\xa0.{4}\\x01\\x80\\xa3.{4}\\x01\\xa1(.{4})", "eye level increment")
        var_movement_eye_level = find_variable(page_data, "\\x80\\xa0.{4}\\x01\\x80\\xa3.{4}\\x01\\xa1.{4}\\x29\\x05(.{4})", "eye level")
        var_movement_eye_level_max = find_variable(page_data, "\\xc1\\xe1\\x0c\\x03\\xc1\\xa3(.{4})", "max eye level")
        var_movement_eye_level_min = find_variable(page_data, "\\x83\\xf8\\x00\\x74\\x1f\\xe8.{4}\\x2b\\x05(.{4})", "min eye level")
        var_movement_eye_level_restore = find_variable(page_data, "\\x2b\\xd0\\x89\\x15(.{4})", "default eye level")
        ...
        if name == "The Pandora Directive":
            var_using_alien_abductor = find_variable(page_data, "\\x88\\x45\\xfc\\xf6\\x45\\xfc\\x02\\x75\\x05\\xe8.{4}\\xe8.{4}\\xc6\\x05(.{4})\\x01", "Alien Abductor flag")
            var_abductor_state = find_variable(page_data, "\\x8b\\x45\\xf0\\x80\\x88.{4}\\x02\\x80\\x3d(.{4})\\x02", "Alien Abductor state")
            var_abductor_dpad = find_variable(page_data, "\\xf7\\xd8\\x89\\x45\\xf8\\xf6\\x05(.{4})\\x04", "Alien Abductor directional pad state")
            var_fake_key_input = find_variable(page_data, "\\xc7\\x45\\xf4\\x00\\x00\\x00\\x00\\xc7\\x45\\xfc(.{4})\\x8b\\x45\\xfc", "Alien Abductor key input buffer")
            var_mouse_unbounded_x_mod = find_variable(page_data, "\\xe9\\x1f\\x02\\x00\\x00\\xc7\\x45\\xfc\\x0c\\x00\\x00\\x00\\x66\\xc7\\x05(.{4})\\x00\\x00\\x66\\xc7\\x05.{4}\\x00\\x00", "Alien Abductor mouse X buffer")
            var_mouse_unbounded_y_mod = find_variable(page_data, "\\xe9\\x1f\\x02\\x00\\x00\\xc7\\x45\\xfc\\x0c\\x00\\x00\\x00\\x66\\xc7\\x05.{4}\\x00\\x00\\x66\\xc7\\x05(.{4})\\x00\\x00", "Alien Abductor mouse Y buffer")
...
ret
"""
        mouselook_offset = find_offset(page_data, "\\x8b\\xc2\\x33\\xed\\x03\\x05.{4}\\x8b\\xd8", 0, "mouselook mod point")
        label_check2 = create_label()
        label_after = create_label()
        mouselook_instrs = assemble_x86([
            Instruction.create_reg_reg(Code.MOV_RM32_R32, Register.EAX, Register.ECX),  
            Instruction.create_reg_u32(Code.SHL_RM32_IMM8, Register.EAX, 17),
            Instruction.create_mem_reg(Code.ADD_RM32_R32, memory(var_movement_rot_angle), Register.EAX),
            Instruction.create_reg_reg(Code.MOV_RM32_R32, Register.EAX, Register.EDX), 
            Instruction.create_reg_u32(Code.SHL_RM32_1, Register.EAX, 1),
            Instruction.create_reg_mem(Code.ADD_R32_RM32, Register.EAX, memory(var_movement_tilt_angle_last)),
            Instruction.create_reg_mem(Code.CMP_R32_RM32, Register.EAX, memory(var_movement_tilt_angle_top)),
            Instruction.create_branch(Code.JGE_REL8_32, label_check2),
            Instruction.create_reg_mem(Code.MOV_EAX_MOFFS32, Register.EAX, memory(var_movement_tilt_angle_top)),
            add_label(label_check2, Instruction.create_reg_mem(Code.CMP_R32_RM32, Register.EAX, memory(var_movement_tilt_angle_bottom))),
            Instruction.create_branch(Code.JLE_REL8_32, label_after),
            Instruction.create_reg_mem(Code.MOV_EAX_MOFFS32, Register.EAX, memory(var_movement_tilt_angle_bottom)),
            add_label(label_after, Instruction.create_mem_reg(Code.MOV_MOFFS32_EAX, memory(var_movement_tilt_angle), Register.EAX)),
            Instruction.create_mem_reg(Code.MOV_MOFFS32_EAX, memory(var_movement_tilt_angle_last), Register.EAX),
...
jmp wasd_rejoin
"""
        wasd_offset = find_offset(page_data, "\\x80\\x3d.{4}\\x00\\x0f\\x84\\x93\\x00\\x00\\x00\\x33\\xc0", 0, "WASD mod point")
        wasd_rejoin = find_offset(page_data, "\\x0f\\xb6\\x1d.{4}\\x80\\xa3.{4}\\x01"*7, 0, "WASD rejoin mod point")
        ...
        abductor_prefix = [] if not var_using_alien_abductor else [
            Instruction.create_mem_i32(Code.CMP_RM8_IMM8, memory(var_using_alien_abductor), 0),
            Instruction.create_branch(Code.JNE_REL8_32, label_skip),
        ]

        wasd_instrs = assemble_x86([
            *abductor_prefix,
            Instruction.create_mem_u32(Code.MOV_RM32_IMM32, memory(var_movement_strafe), 1),
            
            add_label(label_up, Instruction.create_reg_reg(Code.XOR_RM32_R32, Register.EAX, Register.EAX)),
            Instruction.create_mem_u32(Code.TEST_RM8_IMM8, memory(var_keyboard_state+0x11), 3),
            Instruction.create_branch(Code.JE_REL8_32, label_down),
            Instruction.create_reg_i32(Code.SUB_EAX_IMM32, Register.EAX, 0x4000),
            
            add_label(label_down, Instruction.create_mem_i32(Code.TEST_RM8_IMM8, memory(var_keyboard_state+0x1f), 3)),
            ...
            add_label(label_leftyrighty, Instruction.create_mem_i32(Code.TEST_RM8_IMM8, memory(var_keyboard_state+0x2a), 3)),
            Instruction.create_branch(Code.JE_REL8_32, label_apply_fwd),
            Instruction.create_reg_u32(Code.SHL_RM32_1, Register.EAX, 1),
            
            add_label(label_apply_fwd, Instruction.create_mem_reg(Code.MOV_MOFFS32_EAX, memory(var_movement_fwd_veloc_world), Register.EAX)),
            
            add_label(label_left, Instruction.create_reg_reg(Code.XOR_RM32_R32, Register.EAX, Register.EAX)),
            Instruction.create_mem_i32(Code.TEST_RM8_IMM8, memory(var_keyboard_state + 0x1e), 3),
            Instruction.create_branch(Code.JE_REL8_32, label_right),
            Instruction.create_reg_i32(Code.SUB_EAX_IMM32, Register.EAX, 0xc000),
            
            add_label(label_right, Instruction.create_mem_i32(Code.TEST_RM8_IMM8, memory(var_keyboard_state + 0x20), 3)),
            Instruction.create_branch(Code.JE_REL8_32, label_fin),
            Instruction.create_reg_i32(Code.ADD_EAX_IMM32, Register.EAX, 0xc000),
            
            add_label(label_fin, Instruction.create_mem_i32(Code.TEST_RM8_IMM8, memory(var_keyboard_state+0x2a), 3)),
            Instruction.create_branch(Code.JE_REL8_32, label_apply_strafe),
            Instruction.create_reg_i32(Code.SHL_RM32_1, Register.EAX, 1),
            
            add_label(label_apply_strafe, Instruction.create_mem_reg(Code.MOV_MOFFS32_EAX, memory(var_movement_strafe_veloc_world), Register.EAX)),
            Instruction.create_mem_u32(Code.AND_RM8_IMM8, memory(var_keyboard_state+0x11), 1),
            Instruction.create_mem_u32(Code.AND_RM8_IMM8, memory(var_keyboard_state+0x1f), 1),
            Instruction.create_mem_u32(Code.AND_RM8_IMM8, memory(var_keyboard_state+0x1e), 1),
            Instruction.create_mem_u32(Code.AND_RM8_IMM8, memory(var_keyboard_state+0x20), 1),
            Instruction.create_mem_u32(Code.AND_RM8_IMM8, memory(var_keyboard_state+0x2a), 1),
        ...
        wasd_instrs += assemble_x86([
            Instruction.create_branch(Code.JMP_REL32_32, wasd_rejoin - (wasd_offset + len(wasd_instrs)) - 5),
        ])
        wasd_mod_end = len(wasd_instrs) + wasd_offset
        # fill gap with nops
        wasd_instrs += assemble_x86([Instruction.create(Code.NOPD)] * (wasd_rejoin - wasd_mod_end))
...
"""
        rkey_mod_offset = find_offset(page_data, "\\x0f\\xb6\\x1d.{4}\\xf6\\x83.{4}\\x01\\x75\\x0c\\x66\\xb9\\x02\\x00\\x2a\\x0d.{4}\\xd3\\xf8", 0, "R key mod point")
...
"""
        crouch_mod_offset = find_offset(page_data, "\\x0f\\xb6\\x05.{4}\\x0f\\xb6\\x1d.{4}\\xf6\\x80.{4}\\x03", 0, "crouch mod point")
        ...
        crouch_instrs = assemble_x86([
            *abductor_prefix,
            add_label(label_start, Instruction.create_reg(Code.PUSH_R32, Register.ECX)),
            Instruction.create_reg(Code.PUSH_R32, Register.EDX),
            Instruction.create_reg_mem(Code.MOV_R32_RM32, Register.ECX, memory(var_movement_eye_level_min)),
            Instruction.create_reg_mem(Code.ADD_R32_RM32, Register.ECX, memory(var_movement_eye_level_restore)),

            Instruction.create_mem_i32(Code.TEST_RM8_IMM8, memory(var_keyboard_state + 0x2e), 3),
            Instruction.create_branch(Code.JNE_REL8_32, label_crouch),
            Instruction.create_mem_i32(Code.TEST_RM8_IMM8, memory(var_keyboard_state + 0x13), 3),
            Instruction.create_branch(Code.JE_REL8_32, label_restore),
            
            add_label(label_tippytoes, Instruction.create_reg_mem(Code.MOV_EAX_MOFFS32, Register.EAX, memory(var_movement_eye_level_incr))),
            Instruction.create_mem_reg(Code.ADD_RM32_R32, memory(var_movement_eye_level), Register.EAX),
            Instruction.create_reg_mem(Code.MOV_EAX_MOFFS32, Register.EAX, memory(var_movement_eye_level)),
            Instruction.create_reg_mem(Code.CMP_R32_RM32, Register.EAX, memory(var_movement_eye_level_max)),
            Instruction.create_branch(Code.JLE_REL8_32, label_fin),
            Instruction.create_reg_mem(Code.MOV_EAX_MOFFS32, Register.EAX, memory(var_movement_eye_level_max)),
            Instruction.create_mem_reg(Code.MOV_MOFFS32_EAX, memory(var_movement_eye_level), Register.EAX),
            Instruction.create_branch(Code.JMP_REL8_32, label_fin),
            
            add_label(label_crouch, Instruction.create_reg_mem(Code.MOV_EAX_MOFFS32, Register.EAX, memory(var_movement_eye_level_incr))),
            Instruction.create_mem_reg(Code.SUB_RM32_R32, memory(var_movement_eye_level), Register.EAX),
            Instruction.create_reg_mem(Code.MOV_EAX_MOFFS32, Register.EAX, memory(var_movement_eye_level)),
            Instruction.create_reg_mem(Code.CMP_R32_RM32, Register.EAX, memory(var_movement_eye_level_min)),
            Instruction.create_branch(Code.JGE_REL8_32, label_fin),
            Instruction.create_reg_mem(Code.MOV_EAX_MOFFS32, Register.EAX, memory(var_movement_eye_level_min)),
            Instruction.create_mem_reg(Code.MOV_MOFFS32_EAX, memory(var_movement_eye_level), Register.EAX),
            Instruction.create_branch(Code.JMP_REL8_32, label_fin), 
            
            add_label(label_restore,Instruction.create_reg_mem(Code.MOV_EAX_MOFFS32, Register.EAX, memory(var_movement_eye_level))),
            Instruction.create_reg_reg(Code.SUB_R32_RM32, Register.EAX, Register.ECX),
            Instruction.create(Code.CDQ),
            Instruction.create_reg_reg(Code.XOR_RM32_R32, Register.EAX, Register.EDX),
            Instruction.create_reg_reg(Code.SUB_R32_RM32, Register.EAX, Register.EDX),
            Instruction.create_reg_mem(Code.CMP_R32_RM32, Register.EAX, memory(var_movement_eye_level_incr)),
            Instruction.create_branch(Code.JLE_REL8_32, label_skip),
            Instruction.create_reg_mem(Code.MOV_EAX_MOFFS32, Register.EAX, memory(var_movement_eye_level_incr)),
            Instruction.create_reg_mem(Code.CMP_R32_RM32, Register.ECX, memory(var_movement_eye_level)),
            Instruction.create_branch(Code.JG_REL8_32, label_adjust),
            Instruction.create_reg(Code.NEG_RM32, Register.EAX),

            add_label(label_adjust, Instruction.create_mem_reg(Code.ADD_RM32_R32, memory(var_movement_eye_level), Register.EAX)),
            Instruction.create_branch(Code.JMP_REL8_32, label_fin),
            
            add_label(label_skip, Instruction.create_mem_reg(Code.MOV_RM32_R32, memory(var_movement_eye_level), Register.ECX)),
            
            add_label(label_fin, Instruction.create_mem_i32(Code.AND_RM8_IMM8, memory(var_keyboard_state + 0x2e), 1)),
            Instruction.create_mem_i32(Code.AND_RM8_IMM8, memory(var_keyboard_state + 0x13), 1),
...
"""
        ...
        if name == "Under a Killing Moon":
            interactive_draw_frame_offset = find_offset(page_data, "\\x3a\\x05.{4}\\x74\\x22", 0, "interactive frame draw code")
            call1_offset = find_offset(page_data, "\\xe8.{4}\\x9c\\x0f\\xb6\\xc0", 0, "frame call 1")
            call1_instrs = b"\xe8" + utils.to_int32_le(vsync_offset - (call1_offset + 5))
            CODE_PATCHES.append((call1_instrs, call1_offset))
        elif name == "The Pandora Directive":
            interactive_draw_frame_offset = find_offset(page_data, "\\x06\\x60\\x66\\xc7\\x05.{4}\\x00\\x00\\xa8\\x01", 0, "interactive frame draw code")
            call1_offset = find_offset(page_data, "\\xe8.{4}\\x89\\x45\\xf8\\xb8.{4}", 0, "frame call 1")
            call1_instrs = b"\xe8" + utils.to_int32_le(vsync_offset - (call1_offset + 5))
            CODE_PATCHES.append((call1_instrs, call1_offset))
            call2_offset = find_offset(page_data, "\\xe8.{4}\\x89\\x45\\xf4\\xb8.{4}", 0, "frame call 2")
            call2_instrs = b"\xe8" + utils.to_int32_le(vsync_offset - (call2_offset + 5))
        ...
        # add a JMP_REL32_32
        vsync_instrs += b"\xe9" + utils.to_int32_le(interactive_draw_frame_offset - (vsync_offset + len(vsync_instrs) + 5))
        CODE_PATCHES.append((vsync_instrs, vsync_offset))

        if var_using_alien_abductor and var_abductor_state and var_abductor_dpad and var_fake_key_input and var_mouse_unbounded_x_mod and var_mouse_unbounded_y_mod:
            """
...
ret
""" 
            abductor_offset = find_offset(page_data, "\\x53\\x51\\x52\\x56\\x57\\x55\\x89\\xe5\\x81\\xec\\x0c\\x00\\x00\\x00\\xeb\\x10", 0, "Alien Abductor control buttons")
            ...
            abductor_instrs = assemble_x86([
                Instruction.create_mem_i32(Code.CMP_RM8_IMM8, memory(var_fake_key_input), 0x2a),
                ...
                add_label(label_hoverup_write, Instruction.create_mem_reg(Code.MOV_MOFFS32_EAX, memory(var_movement_eye_level), Register.EAX)),

                add_label(label_hoverdown, Instruction.create_mem_i32(Code.CMP_RM8_IMM8, memory(var_fake_key_input), 0x38)),
                ...
                add_label(label_hoverdown_write, Instruction.create_mem_reg(Code.MOV_MOFFS32_EAX, memory(var_movement_eye_level), Register.EAX)),

                add_label(label_dpad, Instruction.create_reg_mem(Code.MOV_AL_MOFFS8, Register.AL, memory(var_abductor_state))),
                Instruction.create_reg_i32(Code.CMP_AL_IMM8, Register.AL, 2),
                Instruction.create_branch(Code.JE_REL8_32, label_move),
                Instruction.create_mem_i32(Code.MOV_RM32_IMM32, memory(var_movement_strafe_veloc_world), 0),
                Instruction.create_mem_i32(Code.MOV_RM32_IMM32, memory(var_movement_fwd_veloc_world), 0),
                Instruction.create_branch(Code.JMP_REL8_32, label_fin),

                add_label(label_move, Instruction.create_mem_i32(Code.TEST_RM8_IMM8, memory(var_abductor_dpad), 0xc)),
                Instruction.create_branch(Code.JE_REL8_32, label_updown),

                add_label(label_turn, Instruction.create_reg_u32(Code.MOV_R32_IMM32, Register.EAX, 0x400000)),
                Instruction.create_mem_i32(Code.TEST_RM8_IMM8, memory(var_abductor_dpad), 0x8),
                Instruction.create_branch(Code.JNE_REL8_32, label_leftright_speed),
                Instruction.create_reg(Code.NEG_RM32, Register.EAX),

                add_label(label_leftright_speed, Instruction.create_mem_i32(Code.TEST_RM8_IMM8, memory(var_keyboard_state + 0x2a), 3)),
                Instruction.create_branch(Code.JE_REL8_32, label_leftright_apply),
                Instruction.create_reg_i32(Code.SHL_RM32_1, Register.EAX, 1),
                add_label(label_leftright_apply, Instruction.create_mem_reg(Code.MOV_MOFFS32_EAX, memory(var_movement_strafe_veloc_world), Register.EAX)),

                add_label(label_updown, Instruction.create_mem_i32(Code.TEST_RM8_IMM8, memory(var_abductor_dpad), 3)),
                Instruction.create_branch(Code.JE_REL8_32, label_fin),
                Instruction.create_reg_u32(Code.MOV_R32_IMM32, Register.EAX, 0x1800),
                Instruction.create_mem_i32(Code.TEST_RM8_IMM8, memory(var_abductor_dpad), 2),
                Instruction.create_branch(Code.JNE_REL8_32, label_updown_speed),
                Instruction.create_reg(Code.NEG_RM32, Register.EAX),

                add_label(label_updown_speed, Instruction.create_mem_i32(Code.TEST_RM8_IMM8, memory(var_keyboard_state + 0x2a), 3)),
                Instruction.create_branch(Code.JE_REL8_32, label_updown_apply),
                Instruction.create_reg_i32(Code.SHL_RM32_1, Register.EAX, 1),

                add_label(label_updown_apply, Instruction.create_mem_reg(Code.MOV_MOFFS32_EAX, memory(var_movement_fwd_veloc_world), Register.EAX)),
                
                add_label(label_fin, Instruction.create_mem_i32(Code.MOV_RM16_IMM16, memory(var_mouse_unbounded_x_mod), 0)),
                Instruction.create_mem_i32(Code.MOV_RM16_IMM16, memory(var_mouse_unbounded_y_mod), 0),
                Instruction.create_mem_i32(Code.AND_RM8_IMM8, memory(var_keyboard_state + 0x2a), 1),
                Instruction.create(Code.RETND),
...
"""
            ...
            abductor_hoverup_offset = find_offset(page_data, "\\x80\\x88.{4}\\x02\\xc6\\x05.{4}\\x00\\xc6\\x05.{4}\\x00\\x31\\xc0\\xe8.{4}\\x80\\x3d.{4}\\x00\\x74\\x1e\\xe8.{4}\\xba\\x01\\x00\\x00\\x00\\xb8\\x04\\x00\\x00\\x00", 0, "Alien Abductor hover-up button")
            ...
            abductor_hoverdown_offset = find_offset(page_data, "\\x80\\x88.{4}\\x02\\xc6\\x05.{4}\\x00\\xc6\\x05.{4}\\x00\\x31\\xc0\\xe8.{4}\\x80\\x3d.{4}\\x00\\x74\\x1e\\xe8.{4}\\xba\\x01\\x00\\x00\\x00\\xb8\\x05\\x00\\x00\\x00", 0, "Alien Abductor hover-down button")
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(description="Apply mods to Under a Killing Moon/The Pandora Directive")
    parser.add_argument("INPUT", type=pathlib.Path, help="Input file name, either tex3.exe or tex4.exe")
    parser.add_argument("OUTPUT", type=pathlib.Path, help="Output file name")
    parser.add_argument("--fix-speed", action="store_true", help="Fix bug where Tex rockets around in areas of low geometric complexity on Pentium/DOSBox.")
    parser.add_argument("--mouselook", action="store_true", help="Replace bonkers movement controls with WASD + mouselook.")

    args = parser.parse_args(argv or sys.argv[1:])
    patch(args.INPUT, args.OUTPUT, args.fix_speed, args.mouselook)
